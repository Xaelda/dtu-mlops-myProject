...
class MyDataset(Dataset):
    ...
    def preprocess(self, output_folder: Path) -> None:
        ...
        for i in range(6):
            train_images.append(torch.load(f"{self.raw_data_pathH}/train_images_{i}.pt"))
            train_target.append(torch.load(f"{self.raw_data_path}/train_target_{i}.pt"))
        ...
        test_images: torch.Tensor = torch.load(f"{self.raw_data_path}/test_images.pt")
        test_target: torch.Tensor = torch.load(f"{self.raw_data_path}/test_target.pt")
